...
router.get('/', authorization, async (req, res) => {
  ...
  try {
    ...
    const myGroup = await pool.query(
      'SELECT * FROM scoreInRounds WHERE sr_group_id != $1 AND (sr_player_one_id = $2 OR sr_player_two_id = $3)',
      [logedUserGropu.rows[0].group_id, currentLogedUser, currentLogedUser]
    );

    console.log(currentLogedUser);
